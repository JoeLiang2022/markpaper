...
- **Config via env vars** (not code): `PORT`, `API_TOKEN`, `BUILD_TIMEOUT`, `MAX_INPUT_BYTES`, `MAX_CONCURRENCY`, `MAX_QUEUE`, `JOB_TTL`, `MAX_STORED_RESULTS`, `SYNC_WAIT_TIMEOUT`, `ENABLE_MERMAID`, `MEM_LIMIT_MB`, `PDF_ENGINE`, `FALLBACK_ENGINE`, `MARKPAPER_ROOT`. Document changes to these in `README.md` and here.

...
- **CJK auto-configuration**: `paper.md` declares its own `CJKmainfont`/`xeCJK`, but arbitrary pasted Markdown does not. `webapp/app.py` writes a `cjk-header.tex` and passes `--include-in-header` only when the document declares no CJK setup of its own; preserve that "don't override the user" behaviour. `--include-in-header` is used deliberately instead of the template's optional `CJKmainfont` variable because it is additive and version-independent.
- **Don't let LaTeX fail silently**: `_scan_log()` turns non-fatal XeLaTeX log entries (dropped glyphs, missing files/fonts) into warnings that reach the job result, the `X-MarkPaper-Warnings` header and the UI. Keep this whenever you touch the xelatex step; `GET /api/diag` exists for the same reason.

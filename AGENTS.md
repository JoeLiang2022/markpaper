...
- **CJK fonts must be installed in the image**: `fonts-noto-cjk` is required by both Dockerfiles (`tools/deps.sh` is local-only and does *not* apply to Docker builds). `tools/detect-fonts.sh` falls back to a hardcoded font name that may not exist, so a missing package silently yields PDFs without Chinese glyphs — both Dockerfiles therefore assert a CJK font is present at build time. Don't remove that check.
- **CJK auto-configuration**: `paper.md` declares its own `CJKmainfont`/`xeCJK`, but arbitrary pasted Markdown does not. `webapp/app.py` injects `-V CJKmainfont=<detected>` only when the document declares no CJK setup of its own; preserve that "don't override the user" behaviour.

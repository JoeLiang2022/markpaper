...
- **Keep the pipeline in sync**: `webapp/app.py`'s `build_pdf()` mirrors `build_pdf()` in `devops.sh` (Mermaid → font detection → font replace → Pandoc+crossref+citeproc → `fix-latex-csl.sh` → LaTeX ×2, XeLaTeX primary with LuaLaTeX as fallback — XeTeX loads fonts through FreeType in C, LuaTeX parses them in Lua and measured ~364 MB peak RSS for a one-line document). If you change the build steps in `devops.sh`, mirror them here (and vice versa).
...
- **Never run the fallback engine on a document it cannot compile**: `_ENGINE_ONLY_PACKAGES`/`_engine_blockers()` skip the retry when the LaTeX source uses engine-specific packages (`xeCJK` is XeTeX-only; `paper.md` loads it from `header-includes`). This is not an optimisation: a doomed LuaTeX run still parses fonts, and on a small instance that OOM-kills the container, so the caller loses both the job and the primary engine's perfectly good error message to a 502.
...
- **Stage logging to stdout is not decoration**: `build_pdf()`'s `stage()` calls are the only diagnosis that survives the container being restarted. When a build exhausts memory, the job (and the error message it carries) is in-memory state that dies with the process, leaving the caller a bare 502 and `Unknown or expired job`. The host's log stream outlives it, so the last line logged names the stage that died and reports peak child RSS. Log *before* each LaTeX pass, not only after — an "after" line is never written if the pass is what died.

...
# End-to-end CJK check. Without this the image can ship "successfully" and only
# fail at request time, which is how a broken font stack reached production.
RUN set -eu; \
    tmp="$(mktemp -d)"; cd "$tmp"; \
    printf '%s\n' \
        ...
        '\begin{document}' \
        '繁體中文測試 Traditional Chinese 123' \
        '\end{document}' > t.tex; \
    if ! xelatex -interaction=nonstopmode -halt-on-error t.tex > out.log 2>&1; then \
        echo "--- xelatex log (tail) ---"; tail -60 out.log; \
        echo "ERROR: CJK smoke test failed to compile"; exit 1; \
    fi; \
    [ -f t.pdf ] || { tail -60 out.log; echo "ERROR: CJK smoke test produced no PDF"; exit 1; }; \
    if grep -q "Missing character" out.log; then \
        grep "Missing character" out.log | head -5; \
        echo "ERROR: CJK glyphs were dropped - font does not cover Chinese"; exit 1; \
    fi; \
    echo "CJK smoke test OK ($(wc -c < t.pdf) bytes)"; \
    cd /; rm -rf "$tmp"

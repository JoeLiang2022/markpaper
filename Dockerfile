...
# path. A hand-written \usepackage{fontspec} file is NOT enough: pandoc's
...
RUN set -eu; \
    ...
    for engine in xelatex lualatex; do \
        ...
        if "$engine" -interaction=nonstopmode t.tex > out.log 2>&1 && [ -f t.pdf ]; then \
            if grep -q "Missing character" t.log 2>/dev/null; then \
                ...
                grep "Missing character" t.log | head -3; \
            else \
    ...
    [ -n "$ok" ] || { echo "ERROR: no LaTeX engine can typeset CJK via pandoc"; exit 1; }; \
    echo "CJK smoke test OK using $ok"; \
    cd /; rm -rf "$tmp"

...
| `PDF_ENGINE`         | `xelatex` | Primary LaTeX engine (cheaper on memory than LuaTeX)           |
| `FALLBACK_ENGINE`    | `lualatex`| Engine retried once if the primary produces no PDF; `""` = off |
...
There is a third, less obvious requirement: **the font has to fit in memory**. LuaTeX's `luaotfload` parses fonts in Lua, and `fonts-noto-cjk` ships a ~120 MB pan-CJK collection that a 512 MB instance cannot afford — the build gets killed even though nothing is missing, and because the instance dies with it you see a **502** rather than an error. Measured on the free plan: a one-line document peaks at **~437 MB RSS** even with a ~10 MB font, so there is very little headroom.

So `Dockerfile.web` also installs the much smaller `fonts-arphic-*` and `fonts-wqy-zenhei` families and defaults `CJK_FONT` to *AR PL UMing TW* (a Ming/serif face; *WenQuanYi Zen Hei* is the sans alternative and also has bold). `CJK_FONT` is set in the image rather than in `render.yaml` on purpose: the image build also warms luaotfload's font cache for that exact family, which requests cannot do themselves because they run with `openout_any=p`. Overriding `CJK_FONT` in the environment works but forfeits that cache. To go back to Noto on a bigger plan, rebuild with `--build-arg CJK_FONT="Noto Sans CJK TC"`.

**When a build takes the instance down.** If a document exhausts memory, the platform restarts the container; the job and its error message were in memory, so all the caller sees is a `502` and then `Unknown or expired job`. Nothing in the API can explain that, because the thing that would explain it died. The service therefore logs each pipeline stage to **stdout** with elapsed time and peak child RSS (`start`, `mermaid`, `pandoc`, `latex-ready`, `xelatex-pass1`, …). Your host's log stream outlives the process, so the last line before the restart names the stage that died — check it in the Render dashboard under *Logs*.

`GET /api/probe` is how you check any of this on a running instance: it compiles a one-line document with each engine and reports exit code, PDF size, **peak child RSS**, whether the run finished or was killed, and the log head/tail. It accepts `font=`, `engine=`, `mem_limit=` and `text=`, so you can compare candidate fonts without rebuilding.

Given those, the service auto-configures CJK: if your Markdown does not declare a font itself, it passes pandoc `-V mainfont=<CJK font>` (`CJK_FONT`, else the detected one), so plain Chinese Markdown with no YAML at all renders correctly. If you *do* want control, declare it yourself and the service leaves your choice alone:
